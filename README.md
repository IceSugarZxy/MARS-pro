...
- 2026-06-30：修复自检前底层读缓冲未清导致贴靠瞬间误判完成的间歇性 bug，新增 S~ 预清理 + readAll() 三层清空
- 2026-06-30：修复串口 TX 排干失败后残留字节堵塞整个写通道的问题，错误发生后自动清 TX 缓冲区并重置写状态
- 2026-06-29：修复位置查询轮询中 clear_data_queue() 引发的竞态条件（丢失硬件响应导致运动序列卡死），撤回该改动
- 2026-06-23：优化位置查询实时性：轮询超时从 1.5s 缩短至 0.3s、查询间隔从 500ms 缩短至 300ms
