# -*- coding: utf-8 -*-
"""MARS 自动重复测量并保存 plot_data。

本脚本放在 ``一致性测试`` 目录下，运行时会自动定位上一级 MARS 项目根目录，
并复用正常 MARS 程序的测量、分析和保存逻辑。

从 MARS 项目根目录运行示例：
    python 一致性测试\auto_measure_save.py

自定义连续测试次数示例：
    python 一致性测试\auto_measure_save.py --count 100 --delay 1

常用参数：
    --count 50    连续测试 50 次；默认 20 次；设为 0 表示一直循环测试
    --delay 2     每次保存完成后等待 2 秒再开始下一次；默认 1 秒

也可以用脚本完整路径运行，不要求当前目录必须在 ``一致性测试`` 中。
"""
...
class AutoMeasureSaveRunner(QObject):
    def __init__(
        ...
        self.measure_timeout_timer = QTimer(self)
        self.measure_timeout_timer.setSingleShot(True)
        self.measure_timeout_timer.timeout.connect(self._on_measure_timeout)
    ...
    def _start_next_measurement(self):
        ...
        self.measure_timeout_timer.stop()
        self.measure_timeout_timer.start(self.measure_timeout_ms)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of cycles to run. Use 0 for infinite loop. Default: 20.",
    )
    parser.add_argument(
        "--delay",
        type=float,
        default=1.0,
        help="Seconds to wait after each save before starting the next measurement. Default: 1.",
    )
    parser.add_argument(
        "--serial-wait",
        type=float,
